...
import { respondUnavailableOnThrow, runNodeInvokeGuard, safeParseJson } from "./nodes.helpers.js";
  ...
  "exec.approvals.node.get": async ({ params, respond, context }) => {
    ...
    await respondUnavailableOnThrow(respond, async () => {
      ...
      const guardResult = await runNodeInvokeGuard(
        context,
        cfg,
        { nodeId: id, command: "system.execApprovals.get", params: {} },
        respond,
      );
  ...
  "exec.approvals.node.set": async ({ params, respond, context }) => {
    ...
    await respondUnavailableOnThrow(respond, async () => {
      ...
      const guardResult = await runNodeInvokeGuard(
        context,
        cfg,
        { nodeId: id, command: "system.execApprovals.set", params: { file, baseHash } },
        respond,
      );
      if (!guardResult.proceed) {
        return;
