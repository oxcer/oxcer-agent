...
import {
  ...
  runActionGuardrail as runActionGuardrailGeneric,
  runResultGuardrail,
} from "../guardrails/action-result.js";
import type { GuardrailOrchestrator, ProposedAction } from "../guardrails/orchestrator.js";

export type ExecToolDefaults = {
  /** OXCER: Optional guardrail orchestrator for pre-exec/post-exec hooks. */
  guardrailOrchestrator?: GuardrailOrchestrator;
  /** OXCER: Resolved guardrails mode (action/result enforce vs warn vs off). */
  guardrailsConfig?: ResolvedGuardrailsConfig;
  host?: ExecHost;
...
/** Exec-specific wrapper: runs generic action guardrail and rethrows with exec-denied message. */
async function runActionGuardrail(
  gConfig: ResolvedGuardrailsConfig | undefined,
  orchestrator: GuardrailOrchestrator | undefined,
  action: ProposedAction,
) {
  try {
    return await runActionGuardrailGeneric(gConfig, orchestrator, action);
  } catch (err) {
    throw new Error(`Exec denied by guardrails: ${(err as Error)?.message ?? err}`);
  }
}
  ...
  return {
    ...
    execute: async (_toolCallId, args, signal, onUpdate) => {
      ...
      if (host === "node") {
        ...
        let requiresAsk = requiresExecApproval({
          ask: hostAsk,
        ...
        const actionForGuardrail: ProposedAction = {
          id: crypto.randomUUID(),
          tool: "exec",
          args: { command: commandText, cwd: workdir, host: "node", nodeId },
          summary: `Run shell command: ${commandText}`,
          sessionKey: defaults?.sessionKey ?? undefined,
          agentId: agentId ?? undefined,
          timestamp: Date.now(),
        };
        const { forceApproval } = await runActionGuardrail(
          defaults?.guardrailsConfig,
          defaults?.guardrailOrchestrator,
          actionForGuardrail,
        );
        requiresAsk = requiresAsk || forceApproval;
        ...
        const resultForGuardrail = { stdout, stderr, exitCode, success, error: errorText };
        const redactResult = await runResultGuardrail(
          defaults?.guardrailsConfig,
          defaults?.guardrailOrchestrator,
          "exec",
          { command: params.command, cwd: workdir, host: "node", nodeId },
          resultForGuardrail,
          { sessionKey: defaults?.sessionKey, agentId },
        );
        const outText = redactResult ? REDACTED_RESULT_TEXT : stdout || stderr || errorText || "";
        return {
          content: [
            {
              type: "text",
      ...
      if (host === "gateway" && !bypassApprovals) {
        ...
        const gatewayCommandText = params.command;
        const gatewayActionForGuardrail: ProposedAction = {
          id: crypto.randomUUID(),
          tool: "exec",
          args: { command: gatewayCommandText, cwd: workdir, host: "gateway" },
          summary: `Run shell command: ${gatewayCommandText}`,
          sessionKey: defaults?.sessionKey ?? undefined,
          agentId: agentId ?? undefined,
          timestamp: Date.now(),
        };
        const { forceApproval: gatewayForceApproval } = await runActionGuardrail(
          defaults?.guardrailsConfig,
          defaults?.guardrailOrchestrator,
          gatewayActionForGuardrail,
        );
        requiresAsk = requiresAsk || gatewayForceApproval;
        ...
        if (requiresAsk) {
          ...
          const noticeSeconds = Math.max(1, Math.round(approvalRunningNoticeMs / 1000));
          const commandText = gatewayCommandText;
      ...
      const directGatewayAction: ProposedAction = {
        id: crypto.randomUUID(),
        tool: "exec",
        args: { command: params.command, cwd: workdir, host: "gateway", sandbox: !!sandbox },
        summary: `Run shell command: ${params.command}`,
        sessionKey: defaults?.sessionKey ?? undefined,
        agentId: agentId ?? undefined,
        timestamp: Date.now(),
      };
      await runActionGuardrail(
        defaults?.guardrailsConfig,
        defaults?.guardrailOrchestrator,
        directGatewayAction,
      );
        ...
        run.promise
          .then(async (outcome) => {
            ...
            const redactResult = await runResultGuardrail(
              ...
              defaults?.guardrailOrchestrator,
              "exec",
              { command: params.command, cwd: run.session.cwd, host: "gateway" },
              resultForGuardrail,
              { sessionKey: defaults?.sessionKey, agentId },
            );
            const outText = redactResult
              ? REDACTED_RESULT_TEXT
