...
fn e2e_session_trace_produces_jsonl_with_expected_components_and_metrics() {
    ...
    let session_path = app_config.join("logs").join(format!("{}.jsonl", SESSION_ID));
    assert!(session_path.exists(), "session log file should exist: {:?}", session_path);

    let content = std::fs::read_to_string(&session_path).unwrap();
    let lines: Vec<&str> = content.lines().filter(|s| !s.is_empty()).collect();
    assert!(!lines.is_empty(), "session log should have at least one line");

    let events: Vec<LogEvent> = lines
        .iter()
        .filter_map(|line| serde_json::from_str::<LogEvent>(line).ok())
        .collect();
    assert_eq!(events.len(), lines.len(), "every line should parse as LogEvent");

    let components: std::collections::HashSet<&str> = events.iter().map(|e| e.component.as_str()).collect();
    assert!(components.contains("semantic_router"), "should have semantic_router event");
    assert!(components.contains("security"), "should have security event");
    assert!(components.contains("llm_client"), "should have llm_client event");
    assert!(components.contains("orchestrator"), "should have orchestrator event");
    ...
    assert!(has_metrics, "at least one event should have non-zero tokens_in, tokens_out, or latency_ms");
}
