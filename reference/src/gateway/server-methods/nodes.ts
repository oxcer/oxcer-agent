...
import { recordGuardrailNodeInvokeComplete } from "../../guardrails/audit.js";
import {
...
export const nodeHandlers: GatewayRequestHandlers = {
  ...
  "node.invoke": async ({ params, respond, context }) => {
    ...
    await respondUnavailableOnThrow(respond, async () => {
      ...
      const guardResult = await runNodeInvokeGuard(
        context,
        cfg,
        { nodeId, command, params: p.params },
        respond,
      );
      if (!guardResult.proceed) {
        return;
      }
      // TODO(Sprint 5): thread guardResult.forceApproval / guardResult.decision for node-level approvals
      ...
      if (!res.ok) {
        recordGuardrailNodeInvokeComplete({
          nodeId,
          command,
          success: false,
          timestamp: Date.now(),
          errorMessage: res.error?.message ?? "node invoke failed",
        });
        respond(
          false,
          undefined,
      ...
      recordGuardrailNodeInvokeComplete({ nodeId, command, success: true, timestamp: Date.now() });
