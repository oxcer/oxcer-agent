...
function defaultRollingPathForToday(): string {
  ...
  const isAppMode = process.env.OXCER_MODE === "app";
  const stateDir = process.env.OPENCLAW_STATE_DIR?.trim() || process.env.OXCER_STATE_DIR?.trim();
  const appLogDir = stateDir
    ? path.join(stateDir, "logs")
    : path.join(os.homedir(), ".oxcer", "logs");
  const dir = isAppMode ? appLogDir : DEFAULT_LOG_DIR;
